...
class Robot:
    ...
    def covariance_drive(self, drive_meas):
        ...
        if ang_vel == 0: # maybe close to 0?
            Jac2[0,0] = np.cos(th)*dt
            Jac2[1,0] = np.sin(th)*dt
        else:
            Jac2[0,0] = (1/ang_vel)*(-np.sin(th)+np.sin(th2))
            Jac2[0,1] = (lin_vel/ang_vel**2)*(ang_vel*dt*np.cos(th2)+np.sin(th)-np.sin(th2))

            Jac2[1,0] = (1/ang_vel)*(np.cos(th)-np.cos(th2))
            Jac2[1,1] = (lin_vel**2/ang_vel**3)*(lin_vel*dt*np.sin(th2)+np.cos(th2)*ang_vel-np.cos(th)*ang_vel)
